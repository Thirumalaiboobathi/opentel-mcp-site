...
Currently live on Cloudflare's free `opentel-mcp-site.pages.dev`
subdomain — no custom domain purchase yet. This is a deliberate,
cost-driven deferral for an unproven project, not a placeholder that
was forgotten; see `DECISIONS.md`'s "Deploy" section for the reasoning.

**Migrating to a custom domain later** (e.g. `opentel-mcp.dev`) is
intentionally cheap, ~15 minutes:

1. Update `SITE.url`/`SITE.domain` in `lib/constants.ts` — every
   canonical URL, JSON-LD `url` field, `llms.txt`/`llms-full.txt`
   (regenerated by `scripts/generate-llms-full.js`, which reads
   `SITE.url` directly rather than hardcoding it), and the sitemap are
   all derived from that one constant.
2. Push. Cloudflare rebuilds automatically.
3. In the Pages project, go to **Custom domains** → **Set up a custom
   domain**, enter the domain, and follow Cloudflare's prompts. If the
   domain's DNS is already on Cloudflare, this is a one-click "Activate
   domain." Cloudflare provisions a free TLS certificate automatically.
4. Set up a 301 redirect from the `.pages.dev` subdomain to the custom
   domain, so any links or citations that accumulated against the
   subdomain URL during the pre-custom-domain period keep working.
