...
<Callout variant="info" title="fingerprinting isn't in this TypeScript interface">
  ...
  false }` even though it's honored at runtime.
  {/* VERIFY: confirm this is still the case in the exact published npm
  tarball for v0.4.0, not just the main branch, and whether it's already
  tracked as a known issue upstream. */}
</Callout>
