...
configuring it through `instrumentMcpServer()`. Still not wired through
as of v0.5.0, which instead added a new top-level `costTracking` option
(see [Cost & Token Attribution](/docs/cost-tracking)); wiring
`classifiers`/`stackFrames` through `instrumentMcpServer()`'s options is
now planned for v0.6.0.
